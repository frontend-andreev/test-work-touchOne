...
export const useApiCall = <T, K, V = undefined>(
  ...
  const error = ref<HTTPError<CamelizeKeys<K>> | null>(null) as Ref<HTTPError<
    CamelizeKeys<K>
  > | null>
  const requestStatus = ref<RequestStatus>(RequestStatus.NOT_STARTED) as Ref<RequestStatus>
  ...
  const executeApiCall = async (externalParams?: V) => {
    ...
    try {
      ...
      const response = await apiCallFunction(abortController, adaptParamsToServer<V>(apiCallParams))
